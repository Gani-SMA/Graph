"""
PyTorch Training Script for GAT+GRU Core Model (gat_gru_v1).
Trains Graph Attention (GAT) + Temporal GRU on strict 80/20 time-based split METR-LA dataset.
Saves model checkpoint to models/gat_gru/gat_gru_v1.pt and prints real measured test metrics.
"""
import os
import torch
...
def train_gat_gru():
    ...
    model = GATGRUCascadeModel(in_features=6, spatial_hidden=32, heads=4, gru_hidden=64, num_horizons=3)
    optimizer = optim.Adam(model.parameters(), lr=0.001)
    criterion = nn.MSELoss()
    
    print(f"Training GAT+GRU Core Model on {X_train.shape[0]} train samples, testing on {X_test.shape[0]} test samples...")
    ...
    # Save Checkpoint
    checkpoint_dir = "models/gat_gru"
    os.makedirs(checkpoint_dir, exist_ok=True)
    ckpt_path = os.path.join(checkpoint_dir, "gat_gru_v1.pt")
    torch.save(model.state_dict(), ckpt_path)
    
    print("=== GAT+GRU Core Model Training Complete ===")
